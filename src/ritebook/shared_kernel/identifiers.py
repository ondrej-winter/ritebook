...
INDEX_NAME_PATTERN = KEBAB_CASE_IDENTIFIER_PATTERN
...
def require_index_name(value: str, *, field_name: str) -> None:
    ...
    if is_index_name(value):
        return
    msg = (
        f"{field_name} must be a single lowercase ASCII kebab-case segment "
        "with no slashes. It must be 1-64 lowercase ASCII letters, digits, "
        "or hyphens; it must not start or end with a hyphen or contain "
        "consecutive hyphens."
    )
    raise ValueError(msg)
